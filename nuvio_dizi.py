...
# --- AYARLAR ---
# Verdiğin link ve yeni kaynak ismi
m3u_sources = [
...
def extract_clean_author(header):
    ...
    # Eğer köşeli parantez yoksa tırnak içindeki son kelimeyi almayı dene
...
def clean_dizi_name_for_alpha(raw_name):
    """Harf tespiti için sadece dizi adını çeker (S01E01 kısmını atar)"""
    # Sezon/Bölüm eklerini (S01E01, 1. Sezon vb.) temizle
        ...
        while i < len(lines):
            ...
            if line.startswith("#EXTINF") and i + 1 < len(lines):
                ...
                if url not in gorulen_url_ler:
                    ...
                    # 1. Kaynak Temizliği (Örn: Zerk)
                    clean_author = extract_clean_author(header_raw)
                    # Header'daki karmaşık author kısmını sadeleştiriyoruz
